...
public class NSUserDefaultsPrefs: Prefs {
    ...
    public func dictionaryForKey(defaultName: String) -> [String : AnyObject]? {
        ...
        let objects =  userDefaults.dictionaryForKey(qualifyKey(defaultName))
        if let strings = objects {
            return strings as! [String : AnyObject]
        }
        return  nil
    ...
    public func clearAll() {
        ...
        for key in userDefaults.dictionaryRepresentation().keys {
            ...
            var keyForString = key as! String
            
            if keyForString.startsWith(prefixWithDot) {
                userDefaults.removeObjectForKey(keyForString)
            }
